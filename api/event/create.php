...
if (
    ...
    try {
        $zoomData = $zoomApp->createMeetingOnZoom($userId, $data);
    } catch (Exception $e) {
    ...
    $event->event_lang = '1';
    $event->max_group_size = trim($data->event_group);
    ...
    if (!is_null($zoomData)) {
        $event->zoom_id = $zoomData->id;
        $event->zoom_password = $zoomData->password;
        $event->zoom_url = $zoomData->join_url;
    }
